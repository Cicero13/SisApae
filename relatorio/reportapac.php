			...
			if($consulta->num_rows > 0) {//se encontrar algum valor
				  ...
				  //topo do lista do ranking
								...
								echo		'<tr>
										...
										echo '<thead>';
								echo		'<tr>
												<th colspan="100%">Profissional: '.$linha['profissional'].' Especialista em '.$linha['especialidade'].'</th>
											</tr>';
									}
											$d1 = explode('-',$datai);
											$d2 = explode('-',$dataf);
								...
								while($linha=$consulta->fetch_array(MYSQLI_ASSOC)){
									...
									echo '<tr>';
										echo '<td>'.$cont.'</td>';
										echo '<td>'.strtoupper($linha['profissional']).'</td>';
										echo '<td>'.$linha['qtd'].'</td>';
										echo '<td>'.$linha['data_ultproced'].'</td>';
